...
if __name__ == "__main__":
    ...
    if not os.path.exists('feature_extraction_cv2'):
        os.makedirs('feature_extraction_cv2')
    ...
    # now select feature dimension and orthogonality_enforcer, which maximize validation accuracy
    ...
    for enforcer in LAMBDAS:
        for num_features in range(10, int(1.0*dim_x), max(1,int(0.1*dim_x))):

            kim = UFDM(dim_x, dim_y, lr=LR, input_projection_dim = num_features, weight_decay=WD, orthogonality_enforcer=enforcer, device=device)         
            mim = MiFeatureExtraction(dim_x, dim_y, lr=LR, input_projection_dim = num_features, weight_decay=WD, orthogonality_enforcer=enforcer, device=device) 
            hsic = MiFeatureExtraction(dim_x, dim_y, lr=LR, input_projection_dim = num_features, weight_decay=WD, orthogonality_enforcer=enforcer, device=device) 
            dcor = MiFeatureExtraction(dim_x, dim_y, lr=LR, input_projection_dim = num_features, weight_decay=WD, orthogonality_enforcer=enforcer, device=device) 
            ...
            for i in range(num_epochs):
                print("Epoch: {}, dim {}/{}, enforcer {}".format(i, num_features, dim_x, enforcer))
            ...
            DCOR_val = dcor.project(torch.from_numpy(X_val.astype(np.float32)).to(device), normalize=normalize).detach().cpu().numpy()
            ...
            rez_DCOR = benchmark(DCOR_train, y_train, DCOR_val, y_val, 'DCORMF')
            ...
            zz.append([num_features, enforcer, rez_raw, rez_kacIMFE, rez_DCOR, rez_MIMFE, rez_HSIC, rez_NCA])

            rez_raw_test = benchmark(X_train, y_train, X_test, y_test, 'R')
            rez_kacIMFE_test = benchmark(F_train, y_train, F_test, y_test, 'KacIMF')
            rez_DCOR_test = benchmark(DCOR_train, y_train, DCOR_test, y_test, 'DCORMF')
            ...
            zz_test.append([num_features, enforcer, rez_raw_test, rez_kacIMFE_test, rez_DCOR_test, rez_MIMFE_test, rez_HSIC_test, rez_NCA_test])

    zz = np.array(zz)
    zz_test = np.array(zz_test)

    if not os.path.isfile('./feature_extraction_cv2/' + sys.argv[1] + '.npy'):
        np.save('./feature_extraction_cv2/' + sys.argv[1] + '.npy', zz_test)
    else:

        zzz = np.load('./feature_extraction_cv2/' + sys.argv[1] + '.npy')
        if zzz.ndim != 2 or zzz.shape[1] != zz_test.shape[1]:
            zzz = zz_test
        else:
            zzz = np.concatenate((zzz,zz_test), axis=0)    
        np.save('./feature_extraction_cv2/' + sys.argv[1] + '.npy', zzz)

    idx_kac = np.argmax(zz[:,3])
    dim_kacimfe = int(zz[idx_kac,0])
    enf_kacimfe = zz[idx_kac,1]

    idx_dcor = np.argmax(zz[:,4])
    dim_dcor = int(zz[idx_dcor,0])
    enf_dcor = zz[idx_dcor,1]

    idx_mi = np.argmax(zz[:,5])
    dim_mi = int(zz[idx_mi,0])
    enf_mi = zz[idx_mi,1]

    idx_hsic = np.argmax(zz[:,6])
    dim_hsic = int(zz[idx_hsic,0])
    enf_hsic = zz[idx_hsic,1]

    idx_nca = np.argmax(zz[:,7])
    dim_nca = int(zz[idx_nca,0])


    kim = UFDM(dim_x, dim_y, lr=LR, input_projection_dim = dim_kacimfe, weight_decay=WD, orthogonality_enforcer=enf_kacimfe, device=device) #0.007    
    mim = MiFeatureExtraction(dim_x, dim_y, lr=LR, input_projection_dim = dim_mi, weight_decay=WD, orthogonality_enforcer=enf_mi, device=device) #0.007
    hsic = MiFeatureExtraction(dim_x, dim_y, lr=LR, input_projection_dim = dim_hsic, weight_decay=WD, orthogonality_enforcer=enf_hsic, device=device) #0.007
    dcor = MiFeatureExtraction(dim_x, dim_y, lr=LR, input_projection_dim = dim_dcor, weight_decay=WD, orthogonality_enforcer=enf_dcor, device=device) #0.007
    ...
    try:
        train = pd.read_csv('feature_extraction_cv2/{}.csv'.format(sys.argv[1]))
        ...
        if train_tensor.shape[0] >= 25:
            ...
            print("A")
            with open('feature_extraction_cv2/{}.csv'.format(sys.argv[1]),'a') as fd:
